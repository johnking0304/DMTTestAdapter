...
namespace TAI.TestDispatcher
{
    ...
    public class BaseCalibrator:BaseController
    {
        ...
        {           
            ...
            byte[] buffer = command.Package();
            this.Controller.UDPClient.SendCommand(buffer);
            this.NotifyMessage(string.Format("通道[{0}]发送标定值[{1}]命令[{2}]", this.ChannelIndex, value, content));
        }
        ...
        public void SaveCalibrateChannelValue()
        {
            ...
            byte[] buffer = command.Package();
            this.Controller.UDPClient.SendCommand(buffer);
            this.NotifyMessage(string.Format("通道[{0}]发送保存标定值命令[{1}]", this.ChannelIndex, content));
        ...
        { 
            this.Notify((int)NotifyEvents.Message, this.ActiveCardModule.Description, "", this, message);
        }
    ...
    public class CalibrateController : BaseController
    {
        ...
        {
            this.UDPClient.UDPClient.Close();
